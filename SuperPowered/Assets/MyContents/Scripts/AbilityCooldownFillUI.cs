...
public class AbilityCooldownFillUI : MonoBehaviour
{
    ...
    private IAbilityCooldown cooldownAbility;
    ...
    private void Awake()
    {
        ...
        cooldownAbility = ability as IAbilityCooldown;

        if (ability != null && cooldownAbility == null)
        {
            Debug.LogWarning($"{ability.name} does not implement IAbilityCooldown.");
        }
    }

    private void Update()
    {
        if (!iconImage || cooldownAbility == null)
            return;

        float cd = cooldownAbility.CooldownDuration;
        float remaining = cooldownAbility.CooldownRemaining;
