...
public class Rage : MonoBehaviour, ICooldownOverrideable, IUpgradeableAbility, IAbilityCooldown
{
    [Header("References")]
    ...
    [SerializeField, Range(0f, 1f)] private float castSoundPlayChance = 1f;
    ...
    [SerializeField] private ParticleSystem hyperdrivePrefab;
    ...
    [SerializeField] private Vector3 hyperVfxScale = Vector3.one;
    [SerializeField] private Vector3 hyperVfxOffset = new Vector3(0f, 0.5f, 1f);
    [SerializeField] private Vector3 hyperVfxRotationOffset = new Vector3(0f, 180f, 0f);
    ...
    private ParticleSystem hyperVFX;
    private float originalMoveSpeed;
    private float newBoostedSpeed;
    ...
    //===== Audio Helper =====//
    private void TryPlayCastSound()
    {
        if (audioSource == null) return;
        if (castSounds == null || castSounds.Length == 0) return;

        if (Random.value > castSoundPlayChance)
            return;
    ...
    private void StartAbility()
    {
        ...
        {
            originalMoveSpeed = agent.speed;
            newBoostedSpeed = agent.speed + boostedMoveSpeed;
            agent.speed = newBoostedSpeed;
        }
    ...
    private void SpawnVFX()
    {
        ...
        //hyperdrive
        if (!hyperdrivePrefab || hyperVFX != null) return;

        Vector3 hyperSpawnPos = transform.TransformPoint(hyperVfxOffset);
        Quaternion hyperSpawnRot = transform.rotation * Quaternion.Euler(hyperVfxRotationOffset);

        hyperVFX = Instantiate(hyperdrivePrefab, hyperSpawnPos, hyperSpawnRot, transform);
        hyperVFX.transform.localScale = hyperVfxScale;
    }
